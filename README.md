...
4. JSON をダウンロードし、**次のいずれかの方法**で認証情報を設定する（上から順に優先）
   1. `--credentials <path>` で明示指定したパス
   2. 環境変数 `GCP_CREDENTIALS_JSON` に JSON の内容全体を設定（`.env` ファイルでも可、下記参照）
   3. 環境変数 `GDRIVE_CREDENTIALS_PATH` のパスに `credentials.json` を配置
   4. カレントディレクトリの `./credentials.json`
   5. `~/.local/share/gdrive-exporter/credentials.json`

#### `.env` で設定する場合

カレントディレクトリの `.env` ファイルは起動時に自動で読み込まれます（既に設定済みの環境変数が優先）。ダウンロードした JSON の内容を 1 行でそのまま貼り付けてください。

```sh
# .env
GCP_CREDENTIALS_JSON='{"installed":{"client_id":"xxx.apps.googleusercontent.com","project_id":"your-project-id","auth_uri":"https://accounts.google.com/o/oauth2/auth","token_uri":"https://oauth2.googleapis.com/token","auth_provider_x509_cert_url":"https://www.googleapis.com/oauth2/v1/certs","client_secret":"xxx","redirect_uris":["http://localhost"]}}'
```

`.env` は `.gitignore` 済みです。コミットしないよう注意してください。
...
# ブラウザが開き、Google アカウントで認証
# （認証情報は GCP_CREDENTIALS_JSON または credentials.json を自動探索）
...
| `--credentials <path>` | credentials.json のパス | `GCP_CREDENTIALS_JSON` → ファイル自動探索（上記の優先順） |
